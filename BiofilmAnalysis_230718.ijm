//Macro BioFilm Analysis (BIAM), developed by Bertrand Cinquin under Fiji (https://fiji.sc/) ,version minimum 1.51k
//Please read the publication PMID: 28679111 DOI: 10.1016/j.mimet.2017.06.021 for a better use.
//Contact me at bcinquin@ens-cachan.fr if further information is necessary or if you have any suggestions.
//The macro offers a segmentation of a biofilm given a multitiff or (z-stack) file of a single labelled fluorescent biofilm into microcolonies.
//For each, structural measurements are given in relation with their intensity.
macro "Biofilm Analysis"{
//OpenFile
...
Dialog.addNumber("Minimum Threshold ", ThrLow);
Dialog.addNumber("Maximum Threshold ", ThrHigh);
Dialog.addCheckbox("Intensity Correction",true);
Dialog.addCheckbox("Binning",true);
Dialog.show();
...
for (n=1; n<=nSlices; n++) {
	setSlice(n);
    run("Measure");
    IntensityArr[n-1] = getResult('Mean');	
      }
...
if (CorrectionState == true){
	run("32-bit");
	pathfile=File.openDialog("Choose the correction file to Open:"); 
...
close("StackMask");
close("StackMask-1");
//Create 2D Connectivity map by projection
...
//Otion1 Manual Threshold on Connectivity map
if(Option == "Manual"){
	Nmb_Connect = 3;
	run("Threshold...");
...
for (j=0; j<=ROIcount-1;j++){
	...
	setBatchMode(false);	
	//CreateSubStack for Filtered measurements
	selectWindow("StackMask"+j);
	run("Divide...", "value=255 stack");
	...
	for (k=0; k<=CountRoi_MaskROI-1 && !exitloop; k++){		
		...
		setThreshold(1,255);
		run("Measure");
		MeanTemp = getResult("Mean",k);print(MeanTemp);
		if (MeanTemp == 0) { 
        	exitloop = true; 
       	} 
       	else { 
	...
	ArrayRoiTemp2= newArray(CountRoi_MaskROI);
	...
	roiManager("Save", Worked_Data_Path+"\\SetROIMask"+j+"_"+Option+"_S"+Nmb_Size+".zip");
	selectWindow("Results");
	IJ.renameResults("Results_"+j);
	selectWindow("SubstackCleaned"+j); run("Close");
}
...
//Saving and organizing images and result tables
SavedResults_Path =Worked_Data_Path+"\\Saved_Results"+File.separator;
File.makeDirectory(SavedResults_Path);
list = getList("image.titles"); 
     for (i=0; i<list.length; i++){ 
     winame = list[i]; 
     selectWindow(winame);
     SUM_Connect_ResultImage = SavedResults_Path+winame+".tif";
     saveAs("Tiff", SUM_Connect_ResultImage);
     run("Close");
     }
 selectWindow("IntensityArr");run("Close");
 selectWindow("ArrayRoiTemp");run("Close");
 if (CorrectionState == true){
 selectWindow("IntensityCorrecSized");run("Close");
 }
//Concatenating tables

for (i=0; i<ROIcount;i++){
	Nonfiltered_windowname = "Results_NONFILTERED_"+i;Filtered_windowname="Results_"+i;
	selectWindow(Nonfiltered_windowname);
	IJ.renameResults(Nonfiltered_windowname,"Results");nlines =nResults; print(nlines);
	Area_Nonfiltered = newArray(nlines);
	Perim_Nonfiltered = newArray(nlines);
	//Place data into Array
	for(z=0;z<nlines; z++){
		Area_Nonfiltered[z]=getResult("Area",z);
		Perim_Nonfiltered[z] =getResult("Perim.",z);
	}
	IJ.renameResults("Results",Nonfiltered_windowname);selectWindow(Nonfiltered_windowname);run("Close");
	selectWindow(Filtered_windowname);IJ.renameResults(Filtered_windowname,"Results");
	//Place data arrays into final table
	for(z=0;z<nlines; z++){
		Table.set("Area_Nonfiltered",z,Area_Nonfiltered[z]);
		Table.set("Perim_Nonfiltered",z,Perim_Nonfiltered[z]);
	}
	IJ.renameResults("Results",Filtered_windowname);
	selectWindow(Filtered_windowname);
	SUM_Connect_ResultTable = SavedResults_Path+Filtered_windowname+".cvs";
	saveAs("Results", SUM_Connect_ResultTable);
	run("Close");
}
}
